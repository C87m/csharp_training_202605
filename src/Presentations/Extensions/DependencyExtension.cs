...
public static class DependencyExtension
{
    ...
    private static void SettingPresentations(IServiceCollection services)
    {
        ...
        // 部署更新ViewModelをドメインオブジェクト:従業員に変換するアダプターインターフェイスの実装
        ...
        services.AddScoped(
            provider =>
            new TempDataStore<EmployeeDeleteViewModel>("EmployeeRegisterViewModel")
        );
        services.AddScoped(
            provider =>
        ...
        services.AddScoped(
            provider =>
            new TempDataStore<DepartmentUpdateViewModel>("EmployeeRegisterViewModel")
        );
        services.AddScoped(
            provider =>
